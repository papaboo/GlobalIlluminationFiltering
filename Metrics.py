...
def SSIM_pr_pixel(noisy_tensor, reference_tensor, filter_std_dev = 1.5):
    ...
    try:
        _, channels, height, width = noisy_tensor.size()
    except:
        channels, height, width = noisy_tensor.size()
    ...
    # TODO Normalize near the border.

    # calculating the mu parameter (locally) for both images using a gaussian filter calculates the luminosity params
    mu_x = F.conv2d(noisy_tensor, gaussian_2D, padding=padding, groups=channels)
    mu_y = F.conv2d(reference_tensor, gaussian_2D, padding=padding, groups=channels)
    ...
    sigma_x_sq = F.conv2d(noisy_tensor * noisy_tensor, gaussian_2D, padding=padding, groups=channels) - mu_x_sq
    sigma_y_sq = F.conv2d(reference_tensor * reference_tensor, gaussian_2D, padding=padding, groups=channels) - mu_y_sq
    sigma_xy =  F.conv2d(noisy_tensor * reference_tensor, gaussian_2D, padding=padding, groups=channels) - mu_xy
